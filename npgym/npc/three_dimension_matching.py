...
def verify_solution(
    triples: List[Tuple[int, int, int]], matching: List[Tuple[int, int, int]], n: int
) -> bool:
    """
    验证给定的matching是否合法
    """
...
print(f"\n解: {solution}")
print(f"验证解是否正确: {verify_solution(all_triples, solution, n)}")

# 验证错误的解
wrong_solution = solution[:-1]  # 移除一个匹配
print(f"\n验证错误的解: {verify_solution(all_triples, wrong_solution, n)}")
